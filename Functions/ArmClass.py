...
# app start games, Printing initialization
def start():
    global __isRunning
    reset()
    __isRunning = True
...
# running Thread
moveObj = Motion()
th = threading.Thread(target=moveObj.Move)
th.setDaemon(True)
th.start()
...
class Perception():
    def __init__(self, img):
        ...
        self.frame_resize = cv2.resize(img_copy, size, interpolation=cv2.INTER_NEAREST)

    def ConvertRGBtoLAB(self):
        frame_gb = cv2.GaussianBlur(self.frame_resize, (11, 11), 11)
    ...
    def FindContour(self, detect_color):
        frame_mask = cv2.inRange(self.frame_lab, color_range[detect_color][0], color_range[detect_color][1])  # mathematical operation on the original image and mask
    ...
    def GetROI(self, area_max, areaMaxContour, img):
        ...
        if not start_pick_up:
            for i in color_range:
                if i in __target_color:
                    detect_color = i  
                    areaMaxContour, area_max = self.FindContour(detect_color)
                    get_roi = self.GetROI(area_max, areaMaxContour, img)
        return img

        

def CallCamera(armObj):
    ...
    while True:
        ...
        if img is not None:
            frame = img.copy()
            Frame = armObj.DrawOutline(frame)           
